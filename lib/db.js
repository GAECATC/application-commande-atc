...
function matchesCredential(candidate, requested) {
  if (!requested) return false;
  if (candidate === requested) return true;
  if (requested.length >= 8 && editDistance(candidate, requested) <= 2) return true;
  return false;
}
...
async function getPartnerByCredentials(id, code) {
  ...
  const requestedPartner = normalizeCredential(id);
  return partners.find((partner) => {
    const matchesIdentifier =
      matchesCredential(normalizeCredential(partner.id), requestedPartner) ||
      matchesCredential(normalizeCredential(partner.name), requestedPartner);
    return matchesIdentifier && partner.code === code && partner.active;
  });
}
