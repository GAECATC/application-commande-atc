...
async function getOrders({ deliveryDate, partnerId } = {}) {
  if (hasSupabase()) {
    const orderFilter = deliveryDate ? `&delivery_date=eq.${deliveryDate}` : "";
    const partnerFilter = partnerId ? `&partner_id=eq.${partnerId}` : "";
    const orders = await supabaseFetch(`orders?select=*&status=eq.active${orderFilter}${partnerFilter}&order=created_at.desc`);
