...
const { isProductVisibleInAvailability } = require("./availability-products");
...
async function migrateLocal(db) {
  ...
  const migrated = {
    ...
    preparationChecklist: db.preparationChecklist || [],
    orders: db.orders || [],
...
function attachPrices(products, prices, priceListId, includeUnpriced = false) {
  ...
  return products
    .filter((product) => includeUnpriced || (priceByProductId.has(product.id) && priceByProductId.get(product.id).listed !== false))
    .map((product) => {
      const record = priceByProductId.get(product.id);
      const listed = Boolean(record && record.listed !== false);
      return { ...product, active: listed || product.active, price: Number(record?.price ?? 0), listed };
    });
}
...
async function getProductAllocations({ partnerId, deliveryDate, inheritPrevious = false }) {
  ...
  if (hasSupabase()) {
    ...
    if (!rows.length && inheritPrevious) {
      rows = await supabaseFetch(
        `product_allocations?select=product_id,quantity,visible&partner_id=eq.${partnerId}&delivery_date=eq.${DEFAULT_AVAILABILITY_DATE}`
      );
    }
    if (!rows.length && inheritPrevious) {
      const previous = await supabaseFetch(
        `product_allocations?select=delivery_date&partner_id=eq.${partnerId}&delivery_date=gt.${DEFAULT_AVAILABILITY_DATE}&delivery_date=lt.${deliveryDate}&order=delivery_date.desc&limit=1`
      );
  ...
  if (!records.length && inheritPrevious) {
    records = (db.productAllocations || []).filter((item) => item.partnerId === partnerId && item.deliveryDate === DEFAULT_AVAILABILITY_DATE);
  }
  if (!records.length && inheritPrevious) {
    const previousDate = (db.productAllocations || []).filter((item) => item.partnerId === partnerId && item.deliveryDate > DEFAULT_AVAILABILITY_DATE && item.deliveryDate < deliveryDate)
...
async function validateProductAllocations({ partnerId, deliveryDate, items, excludeOrderId }) {
  ...
  const products = partner ? await getProducts({ includeHidden: true, priceListId: partner.priceListId }) : [];
  ...
  for (const item of items) {
    ...
    const productName = productNameById.get(item.productId) || item.productName || item.productId;
    if (!isProductVisibleInAvailability(productById.get(item.productId) || {}, allocationByProduct)) {
      throw new Error(`Le produit « ${productName} » n’est pas disponible pour ce client`);
    }
    const allocated = Number(allocation?.quantity || 0);
    if (allocated <= 0) continue;
    const remaining = Math.max(0, allocated - (alreadyOrdered.get(item.productId) || 0));
    if (requested > remaining) throw new Error(`Disponibilité insuffisante pour « ${productName} » (${remaining} ${productNameById.has(item.productId) ? (products.find((product) => product.id === item.productId)?.unit || "") : ""} restant)`);
  }
}
...
async function getProducts({ includeHidden = false, priceListId } = {}) {
  ...
  if (hasSupabase()) {
    const filter = includeHidden || priceListId ? "" : "&active=eq.true";
  ...
  const products = db.products
    ...
    .filter((product) => includeHidden || priceListId || product.active)
...
async function upsertProduct(product) {
  ...
  if (product.priceListId) {
    ...
    const priceRecord = { priceListId: product.priceListId, productId: normalized.id, price: Number(product.price), listed: db.productPrices[priceIndex]?.listed !== false };
...
async function upsertProductPrice(priceListId, productId, price) {
  ...
  if (hasSupabase()) {
    await supabaseFetch("product_prices?on_conflict=price_list_id,product_id", {
      method: "POST",
      headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
      body: JSON.stringify({ price_list_id: priceListId, product_id: productId, price, listed: true })
    });
    return;
  }
  ...
  const record = { priceListId, productId, price: Number(price), listed: true };
...
async function deleteProductPrice(priceListId, productId) {
  ...
  if (hasMySql()) return mysqlDb.deleteProductPrice(priceListId, productId);
  if (hasSupabase()) {
    await supabaseFetch(`product_prices?price_list_id=eq.${encodeURIComponent(priceListId)}&product_id=eq.${encodeURIComponent(productId)}`, {
      method: "PATCH",
      headers: { Prefer: "return=minimal" },
  ...
  const db = await readLocal();
  db.productPrices = db.productPrices.map((price) =>
    price.priceListId === priceListId && price.productId === productId ? { ...price, listed: false } : price
  );
  await writeLocal(db);
  return { priceListId, productId };
}
...
async function deleteProduct(productId) {
  ...
  const basket = baskets.find((item) => item.items.some((line) => line.productId === productId));
  if (basket) throw new Error(`Produit utilisé dans le panier « ${basket.name} ». Retirez-le d'abord de ce panier avant de le supprimer.`);

  const activeOrders = await getOrders();
  if (activeOrders.some((order) => order.items.some((item) => item.productId === productId))) {
    throw new Error("Produit présent dans une commande en cours. Validez ou modifiez cette commande avant de supprimer le produit.");
  }
...
async function getPreparationChecks(deliveryDate) {
  if (hasMySql()) return mysqlDb.getPreparationChecks(deliveryDate);
  if (hasSupabase()) {
    const rows = await supabaseFetch(`preparation_checklist?select=item_key&delivery_date=eq.${deliveryDate}`);
    return rows.map((row) => row.item_key);
  }
  const db = await readLocal();
  return (db.preparationChecklist || []).filter((item) => item.deliveryDate === deliveryDate).map((item) => item.itemKey);
}

async function setPreparationCheck({ deliveryDate, itemKey, checked }) {
  if (hasMySql()) return mysqlDb.setPreparationCheck({ deliveryDate, itemKey, checked });
  if (hasSupabase()) {
    if (checked) await supabaseFetch("preparation_checklist?on_conflict=delivery_date,item_key", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify({ delivery_date: deliveryDate, item_key: itemKey }) });
    else await supabaseFetch(`preparation_checklist?delivery_date=eq.${deliveryDate}&item_key=eq.${encodeURIComponent(itemKey)}`, { method: "DELETE", headers: { Prefer: "return=minimal" } });
    return { deliveryDate, itemKey, checked: Boolean(checked) };
  }
  const db = await readLocal();
  db.preparationChecklist = (db.preparationChecklist || []).filter((item) => item.deliveryDate !== deliveryDate || item.itemKey !== itemKey);
  if (checked) db.preparationChecklist.push({ deliveryDate, itemKey });
  await writeLocal(db);
  return { deliveryDate, itemKey, checked: Boolean(checked) };
}

async function clearPreparationChecks(deliveryDate) {
  if (hasMySql()) return mysqlDb.clearPreparationChecks(deliveryDate);
  if (hasSupabase()) return supabaseFetch(`preparation_checklist?delivery_date=eq.${deliveryDate}`, { method: "DELETE", headers: { Prefer: "return=minimal" } });
  const db = await readLocal();
  db.preparationChecklist = (db.preparationChecklist || []).filter((item) => item.deliveryDate !== deliveryDate);
  await writeLocal(db);
}
