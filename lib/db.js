...
function attachPrices(products, prices, priceListId, includeUnpriced = false) {
  ...
  return products
    ...
    .map((product) => ({
      ...product,
      active: priceByProductId.has(product.id) || product.active,
      price: priceByProductId.get(product.id) ?? 0,
      listed: priceByProductId.has(product.id)
    }));
}
...
async function deleteProductPrice(priceListId, productId) {
  ...
  const db = await readLocal();
  db.productPrices = db.productPrices.filter(
    (price) => price.priceListId !== priceListId || price.productId !== productId
  );
  await writeLocal(db);
  return { priceListId, productId };
}
