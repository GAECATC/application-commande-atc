...
function buildCrateSummary(orders) {
  ...
  for (const order of orders) for (const item of order.items || []) {
    ...
    const key = isSalad ? `salads:${type}` : JSON.stringify([item.productId || name, unit, type]);
    const row = groups.get(key) || { id: key, name: isSalad ? "Laitues" : item.productName, unit: item.unit, type, capacity, isSalad, quantity: 0 };
