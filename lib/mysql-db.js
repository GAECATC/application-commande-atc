...
const { buildBasketEditMetadataItem, buildBasketMetadataItems, isBasketMetadataItem, splitBasketMetadata } = require("./order-baskets");

let pool;
let orderCommentColumnReady;
let basketTablesReady;
let clientGroupTablesReady;
let availabilityMessageTableReady;
let preparationChecklistTableReady;
const DEFAULT_AVAILABILITY_DATE = "1900-01-01";
...
function normalizeProduct(row) {
  return {
    ...
    active: Boolean(row.listed) || (row.active !== 0 && row.active !== false),
    sortOrder: Number(row.sort_order || 0),
    price: Number(row.price || 0),
    listed: row.listed === undefined ? undefined : Boolean(row.listed)
  };
}

function normalizeOrder(row, items) {
  const split = splitBasketMetadata(items);
  return {
    id: row.id,
    ...
    basketCompositionEdited: split.basketCompositionEdited
  };
}
    ...
    basketTablesReady = (async () => {
      const existingTables = await query(
        `select table_name from information_schema.tables
         where table_schema = database() and table_name in ('basket_templates', 'basket_template_items')`
      );
      if (existingTables.length === 2) return;
...
async function ensureClientGroupTables() {
  if (!clientGroupTablesReady) {
    clientGroupTablesReady = (async () => {
      await query(`create table if not exists client_groups (
        id varchar(191) primary key,
        name varchar(255) not null
      ) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci`);
      await query(`create table if not exists client_group_members (
    ...
    })().catch((error) => { clientGroupTablesReady = null; throw error; });
  }
  return clientGroupTablesReady;
}

async function ensureAvailabilityMessageTable() {
  if (!availabilityMessageTableReady) availabilityMessageTableReady = query(`create table if not exists availability_messages (
    delivery_date date not null, partner_id varchar(191) not null, message text not null,
  ...
  ) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci`).catch((error) => { availabilityMessageTableReady = null; throw error; });
  return availabilityMessageTableReady;
}

async function ensurePreparationChecklistTable() {
  if (!preparationChecklistTableReady) preparationChecklistTableReady = (async () => {
    const existing = await query(
      `select 1 from information_schema.tables
       where table_schema = database() and table_name = 'preparation_checklist'
       limit 1`
    );
    if (existing.length) return;
    await query(`create table preparation_checklist (
      delivery_date date not null, item_key varchar(512) not null,
      primary key (delivery_date, item_key)
    ) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci`);
  })().catch((error) => { preparationChecklistTableReady = null; throw error; });
  return preparationChecklistTableReady;
}

async function getPreparationChecks(deliveryDate) {
  await ensurePreparationChecklistTable();
  const rows = await query("select item_key from preparation_checklist where delivery_date = ?", [deliveryDate]);
  return rows.map((row) => row.item_key);
}

async function setPreparationCheck({ deliveryDate, itemKey, checked }) {
  await ensurePreparationChecklistTable();
  if (checked) await getPool().execute("insert ignore into preparation_checklist (delivery_date, item_key) values (?, ?)", [deliveryDate, itemKey]);
  else await getPool().execute("delete from preparation_checklist where delivery_date = ? and item_key = ?", [deliveryDate, itemKey]);
  return { deliveryDate, itemKey, checked: Boolean(checked) };
}

async function clearPreparationChecks(deliveryDate) {
  await ensurePreparationChecklistTable();
  await getPool().execute("delete from preparation_checklist where delivery_date = ?", [deliveryDate]);
}
...
async function getPartnerByCredentials(id, code) {
  const rows = await query("select * from partners where id = ? and binary code = binary ? and active = 1 limit 1", [id, code]);
  ...
  try {
    ...
    if (!rows.length && inheritPrevious) {
      rows = await query(
        `select product_id, quantity, visible from product_allocations
         where partner_id = ? and delivery_date = ?`,
        [partnerId, DEFAULT_AVAILABILITY_DATE]
      );
    }
    if (!rows.length && inheritPrevious) {
      const previousDates = await query(
        `select delivery_date from product_allocations
         where partner_id = ? and delivery_date > ? and delivery_date < ?
         group by delivery_date order by delivery_date desc limit 1`,
        [partnerId, DEFAULT_AVAILABILITY_DATE, deliveryDate]
      );
      if (previousDates[0]?.delivery_date) {
        rows = await query(
          `select product_id, quantity, visible from product_allocations
           where partner_id = ? and delivery_date = ?`,
          [partnerId, previousDates[0].delivery_date]
        );
      }
    }
    return rows.map((row) => ({
...
async function getAvailabilityMessage({ partnerId, deliveryDate }) {
  await ensureAvailabilityMessageTable();
  const rows = await query("select message from availability_messages where partner_id = ? and delivery_date = ?", [partnerId, deliveryDate]);
  return rows[0]?.message || "";
}

async function saveAvailabilityMessage({ partnerId, deliveryDate, message }) {
  await ensureAvailabilityMessageTable();
  await getPool().execute(`insert into availability_messages (delivery_date, partner_id, message) values (?, ?, ?)
    on duplicate key update message = values(message)`, [deliveryDate, partnerId, String(message || "").trim()]);
...
async function getClientGroups() {
  await ensureClientGroupTables();
  const [groups, members] = await Promise.all([
    query("select id, name from client_groups order by name asc"),
    query("select group_id, partner_id from client_group_members order by partner_id asc")
  ]);
...
async function upsertClientGroup(group) {
  ...
  try {
    await connection.beginTransaction();
    await connection.execute(
      "insert into client_groups (id, name) values (?, ?) on duplicate key update name = values(name)",
      [record.id, record.name]
    );
    await connection.execute("delete from client_group_members where group_id = ?", [record.id]);
    for (const partnerId of record.memberIds) {
      await connection.execute("insert into client_group_members (group_id, partner_id) values (?, ?)", [record.id, partnerId]);
    }
    await connection.commit();
    return record;
  } catch (error) {
...
async function deleteClientGroup(id) {
  await ensureClientGroupTables();
  const [result] = await getPool().execute("delete from client_groups where id = ?", [id]);
  if (!result.affectedRows) throw new Error("Groupe introuvable");
  return { id };
}
...
function buildProductsQuery({ includeHidden = false, priceListId } = {}) {
  const params = [];
  let sql = "select p.*";
  if (priceListId) sql += ", pp.price, coalesce(pp.listed, 0) as listed";
  sql += " from products p";
  if (priceListId) {
    sql += includeHidden
      ? " left join product_prices pp on pp.product_id = p.id and pp.price_list_id = ?"
      : " inner join product_prices pp on pp.product_id = p.id and pp.price_list_id = ? and pp.listed = 1";
    params.push(priceListId);
  }
  if (!includeHidden && !priceListId) sql += " where p.active = 1";
  sql += " order by p.sort_order asc";
  return { sql, params };
}

async function getProducts(options = {}) {
  const { sql, params } = buildProductsQuery(options);
...
async function upsertProductPrice(priceListId, productId, price) {
  await query(
    `insert into product_prices (price_list_id, product_id, price, listed)
     values (?, ?, ?, 1)
     on duplicate key update price = values(price), listed = 1`,
    [priceListId, productId, Number(price)]
  );
}

async function deleteProductPrice(priceListId, productId) {
  if (!priceListId || !productId) throw new Error("Grille tarifaire et produit requis");
  await query("update product_prices set listed = 0 where price_list_id = ? and product_id = ?", [priceListId, productId]);
...
async function createOrder({ partnerId, deliveryDate, harvestDay, items, comment, basketSnapshots = [], allowedProductIds = [] }) {
  ...
  try {
    ...
    await insertOrderItems(connection, [...orderItems, ...metadataItems]);
    await connection.commit();
  } catch (error) {
    await connection.rollback();
    throw error;
...
async function updateOrder({ orderId, partnerId, items, comment, allowedProductIds = [] }) {
  ...
  try {
    ...
    const [existingItemRows] = await connection.execute("select * from order_items where order_id = ?", [orderId]);
    const basketMetadataItems = existingItemRows.map(normalizeOrderItem).filter(isBasketMetadataItem);
    if (basketMetadataItems.length && !basketMetadataItems.some((item) => item.category === "__basket_edit__")) {
      basketMetadataItems.push(buildBasketEditMetadataItem(orderId));
    }
    ...
    await insertOrderItems(connection, [...orderItems, ...basketMetadataItems]);
    await connection.commit();
    return { ...normalizeOrder(orders[0], [...orderItems, ...basketMetadataItems]), total };
  } catch (error) {
    await connection.rollback();
    throw error;
  } finally {
