...
async function createOrder({ partnerId, deliveryDate, harvestDay, items }) {
  ...
  try {
    ...
    await connection.execute(
      `insert into orders (id, partner_id, delivery_date, harvest_day, status, created_at, total)
       values (?, ?, ?, ?, ?, ?, ?)`,
      [order.id, order.partnerId, order.deliveryDate, order.harvestDay, order.status, toMySqlDateTime(order.createdAt), order.total]
    );
