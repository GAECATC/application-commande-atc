...
async function ensurePreparationChecklistTable() {
  if (!preparationChecklistTableReady) preparationChecklistTableReady = query(`create table if not exists preparation_checklist (
    delivery_date date not null, item_key varchar(512) not null,
    primary key (delivery_date, item_key)
  ) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci`).catch((error) => { preparationChecklistTableReady = null; throw error; });
