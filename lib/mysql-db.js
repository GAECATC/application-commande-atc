...
const { buildBasketMetadataItems, splitBasketMetadata } = require("./order-baskets");
...
async function createOrder({ partnerId, deliveryDate, harvestDay, items, comment, basketSnapshots = [] }) {
  await ensureOrderCommentColumn();
  const allowedBasketProductIds = new Set(basketSnapshots.flatMap((basket) => basket.items || []).map((item) => item.productId));
