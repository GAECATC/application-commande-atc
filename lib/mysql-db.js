...
function buildProductsQuery({ includeHidden = false, priceListId } = {}) {
  const params = [];
  let sql = "select p.*";
  if (priceListId) sql += ", pp.price, (pp.product_id is not null) as listed";
  sql += " from products p";
  if (priceListId) {
    sql += includeHidden
...
async function deleteProductPrice(priceListId, productId) {
  ...
  await query("delete from product_prices where price_list_id = ? and product_id = ?", [priceListId, productId]);
