...
function classifyMailError(error) {
  ...
  if (code === "ETIMEDOUT") return "smtp-timeout";
  if (code === "EAUTH") return "smtp-authentication";
  if (code === "EDNS") return "smtp-dns";
  if (code === "EENVELOPE" || responseCode === 550 || responseCode === 551 || responseCode === 553) return "recipient-rejected";
  if (["ECONNECTION", "ECONNREFUSED", "ECONNRESET", "ESOCKET"].includes(code)) return "smtp-connection";
  return "smtp-unknown";
}
...
module.exports = { buildAdminOrderEmail, buildOrderEmail, classifyMailError, sendAdminOrderAlert, sendAvailabilityNotice, sendOrderConfirmation };
