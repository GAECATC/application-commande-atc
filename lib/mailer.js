...
function itemLabel(item) {
  return `${item.productName}: ${formatNumber(item.quantity)} ${unitLabel(item.unit)} - ${formatCurrency(item.quantity * item.unitPrice)}`;
}
...
function buildChangeSummary(previousOrder, order) {
  ...
  for (const previousItem of previousOrder.items) {
    ...
    if (!nextItem) {
      changes.push(`- ${previousItem.productName}: supprimé (${formatNumber(previousItem.quantity)} ${unitLabel(previousItem.unit)} - ${formatCurrency(previousItem.quantity * previousItem.unitPrice)})`);
    } else if (Number(previousItem.quantity) !== Number(nextItem.quantity)) {
      changes.push(`- ${previousItem.productName}: ${formatNumber(previousItem.quantity)} ${unitLabel(previousItem.unit)} - ${formatCurrency(previousItem.quantity * previousItem.unitPrice)} -> ${formatNumber(nextItem.quantity)} ${unitLabel(nextItem.unit)} - ${formatCurrency(nextItem.quantity * nextItem.unitPrice)}`);
    }
  }

  for (const nextItem of order.items) {
    if (!previousByProductId.has(nextItem.productId)) {
      changes.push(`- ${nextItem.productName}: ajouté (${formatNumber(nextItem.quantity)} ${unitLabel(nextItem.unit)} - ${formatCurrency(nextItem.quantity * nextItem.unitPrice)})`);
    }
  }
...
function buildOrderEmail({ partner, order, previousOrder, mode = "created" }) {
  ...
  const totalLabel = mode === "updated" ? "Nouveau total" : isCancelled ? "Total de la commande supprimée" : "Total estimé";
  textParts.push("", `${totalLabel}: ${formatCurrency(order.total)}`, "", closing, "", ...signatureText());
  const text = textParts.join("\n");

  const htmlLines = order.items.map(
    (item) => `<li>${escapeHtml(item.productName)}: <strong>${formatNumber(item.quantity)} ${unitLabel(item.unit)} - ${formatCurrency(item.quantity * item.unitPrice)}</strong></li>`
  ).join("");
