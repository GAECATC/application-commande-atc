...
function planTariffRecovery(reference, lists, products, prices) {
  ...
  for (const [sourceName, sourceRows] of Object.entries(reference)) {
    ...
    const review = [];
    let preserved = 0;
    for (const { row, candidates, unitConflict } of matchTariffRows(sourceRows, products)) {
      if (candidates.length !== 1) {
        review.push(`${row.name} : ${candidates.length} correspondance(s)${unitConflict ? ", unité différente" : ""}`);
        continue;
      }
